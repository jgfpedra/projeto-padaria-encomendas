    ...
    if request.method == 'POST':
        ...
        if id:  # If id is provided, check if it exists and then update or insert
            ...
            if existing_client:
                ...
                cur.execute('''
                    UPDATE impressora
                    SET loja = %s, descricao = %s, endereco = %s, tipo_impressao = %s, modelo_impressora = %s, porta = %s, utiliza_guilhotina = %s
                    WHERE id = %s
                ''', (loja, descricao, endereco, tipo_impressao, modelo_impressora, porta, utiliza_guilhotina, id))
                flash('Impressora atualizada com sucesso', 'success')
            else:
                # Insert a new record if the ID does not exist
                insert_impressora(loja, descricao, endereco, tipo_impressao, modelo_impressora, porta, utiliza_guilhotina)
...
def insert_impressora(loja, descricao, endereco, tipo_impressao, modelo_impressora, porta, utiliza_guilhotina):
    conn = get_db_connection()
    cur = conn.cursor()

    print(loja, descricao, endereco, tipo_impressao, modelo_impressora, porta,
          utiliza_guilhotina)
    cur.execute('''
        INSERT INTO impressora (loja, descricao, endereco, tipo_impressao, modelo_impressora, porta, utiliza_guilhotina)
        VALUES (%s, %s, %s, %s, %s, %s, %s)
    ''', (loja, descricao, endereco, tipo_impressao, modelo_impressora, porta, utiliza_guilhotina))
